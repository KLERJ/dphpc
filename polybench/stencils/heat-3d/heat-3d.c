...
static void init_array(int n, DATA_TYPE POLYBENCH_3D(A, N, N, N, n, n, n),
  ...
  for (i = 0; i < n; i++)
    for (j = 0; j < n; j++)
      for (k = 0; k < n; k++)
        A[i][j][k] = B[i][j][k] = (DATA_TYPE)(i * i + j + (n - k)) * 10 / (n);
}
...
int main(int argc, char **argv) {
  ...
  // FILE *out_ptr = fopen("/cluster/scratch/jmandula/signle_thread1008","wb");
  //FILE *out_ptr = fopen("binary_dump_600x30", "wb");

  //fwrite(A, sizeof(DATA_TYPE), n * n * n, out_ptr);
